...
void sFft(float *fftBuffer, long fftFrameSize, long sign);
double sAtan2(double x, double y);
...
void PitchShift(float pitchShift, long numSampsToProcess, long fftFrameSize, long osamp, float sampleRate, float *indata, float *outdata)
{
	...
	for (i = 0; i < numSampsToProcess; i++){
		...
		if (gRover >= fftFrameSize) {
			...
			/* ***************** ANALYSIS ******************* */
			/* do transform */
            sFft(gFFTworksp, fftFrameSize, -1);
...
/*
    ...
    instances of atan2() with calls to the sAtan2() function below.
